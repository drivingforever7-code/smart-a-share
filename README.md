...
仓库包含 `render.yaml` 和 `Dockerfile`，可在 Render 中选择 Blueprint 并连接本仓库部署。
首次创建时将 `DEEPSEEK_API_KEY` 填入 Render 的 Secret 环境变量；不要写入代码或提交 `.env`。
免费实例会休眠且本地 SQLite 不持久，适合公开试用，不适合作为长期生产数据库。
